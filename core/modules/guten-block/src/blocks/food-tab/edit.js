...
wp.apiFetch({path: "wc/store/products/categories"}).then(categories => {
	if(categories.length < 1 ){productCategories.push({ label: __('No Categories Found', 'wpcafe'), value: null });}
	emptyProducts = false;
...
export default function Edit(props) {
			...
			<PanelBody title={ __( 'Settings' , 'wpcafe' )}>
				...
				<PanelRow>
					<ToggleControl
						...
						onChange={(show_thumbnail) => setAttributes({
				...
				<PanelRow>
					<SelectControl
						...
						value = {attributes.wpc_food_categories}
						options = {productCategories}
						onChange={ ( cats ) => {
							setAttributes( { wpc_food_categories: cats } )
						}
						}
					/>
				</PanelRow>
				...
				<PanelRow>
					<ToggleControl
						...
						checked={attributes.wpc_cart_button == "yes" ? true : false}
						onChange={(cart_button) => setAttributes({
							wpc_cart_button: cart_button ? 'yes': 'no'
						})}
					/>
				</PanelRow>
				<PanelRow>
	...
	return [
		<div className={ props.className }>
			...
			{emptyProducts &&
			<p>Add at least one product category.</p>
			}
		</div>
	];
}
